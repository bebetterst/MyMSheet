          ...
          <div className="border rounded-lg p-4 space-y-3">
            ...
            <div className="flex items-center gap-3">
              <Select
                ...
                onValueChange={(val) => setSelectedViewId(val === 'none' ? '' : val)}
              >
              ...
              <Button variant="outline" onClick={handleApplySelectedView}>
                应用视图
              </Button>
              <Button variant="outline" onClick={handleSetDefaultView}>
                设为默认
              </Button>
            </div>
          </div>
          ...
          <div className="flex-1 overflow-hidden">
            ...
            <ScrollArea className="h-[32vh] pr-4">
              <div className="space-y-2">
                {views.map((v) => (
                  ...
                  >
                    ...
                    <div className="flex items-center space-x-2">
                      ...
                      />
                      <Button variant="outline" size="sm" onClick={() => handleRenameView(v.id)}>
                        重命名
                      </Button>
                      <Button variant="outline" size="sm" onClick={() => handleDeleteView(v.id)}>
                        删除
                      </Button>
                      <Button variant="outline" size="sm" onClick={() => applySavedView(v.id)}>
                        应用
                      </Button>
                    </div>
                  </div>
                ...
                {views.length === 0 && (
                  <div className="text-xs text-gray-500">暂无命名视图，先保存一个吧。</div>
                )}
              </div>
            </ScrollArea>
          </div>
