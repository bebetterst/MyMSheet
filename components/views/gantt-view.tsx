...
import { format, addDays, differenceInDays, startOfWeek, endOfWeek, eachDayOfInterval, isSameDay, startOfMonth, endOfMonth, startOfQuarter, endOfQuarter, startOfYear, endOfYear, addWeeks, addMonths, addQuarters, addYears, isWeekend, differenceInBusinessDays } from "date-fns"
...
import { Settings, ZoomIn, ZoomOut, BarChart2, Printer, Plus, ChevronRight, ChevronDown, Search, Filter, RotateCcw, ArrowLeft, ArrowRight, Maximize2 } from "lucide-react"
...
export function GanttView() {
  ...
  const columnWidth = useMemo(() => {
    const baseWidth = viewSettings.timeScale === 'year' ? 100 :
                      viewSettings.timeScale === 'quarter' ? 80 :
                      viewSettings.timeScale === 'month' ? 60 :
                      viewSettings.timeScale === 'week' ? 40 : 50
  ...
  const { startDate, endDate, periods } = useMemo(() => {
    // 获取所有任务的日期范围
    const dates = processedTasks.flatMap(t => {
      const d = []
      // 使用配置的字段获取日期
    ...
    while (current <= end) {
      ...
      switch (viewSettings.timeScale) {
        case 'year': current = addYears(current, 1); break;
        case 'quarter': current = addQuarters(current, 1); break;
  ...
  }, [processedTasks, fieldMapping.startDate, fieldMapping.endDate, viewSettings.timeScale, viewSettings.showWeekend])
    ...
    groups.forEach(group => {
      ...
      if (isExpanded) {
        group.tasks.forEach(task => {
          // 获取任务时间
          const startStr = (task as any)[fieldMapping.startDate] || task.fields?.[fieldMapping.startDate]
  ...
  const dependencyLines = useMemo(() => {
    ...
    const taskMap = new Map(renderData.rows.filter(r => r.type === 'task' && r.hasDates).map(r => [r.data.id, r]))
  ...
  const getTaskProgress = (task: any) => {
    ...
    let val = task[progressField] ?? task.fields?.[progressField] ?? task.customFields?.[progressField]?.value
  ...
  const scrollToTask = (taskId: string) => {
    ...
    const taskRow = renderData.rows.find(r => r.type === 'task' && r.data.id === taskId && r.hasDates)
  ...
  return (
    <div className="flex h-full flex-col bg-white rounded-lg shadow-sm border overflow-hidden">
      ...
      <div className="flex items-center justify-between p-2 border-b bg-gray-50/50">
        <div className="flex items-center gap-2">
          ...
          <div className="flex items-center border rounded-md bg-white shadow-sm h-8 px-2 gap-2">
             <ZoomOut className="h-4 w-4 text-gray-500 cursor-pointer hover:text-blue-600" onClick={() => handleZoom(-10)} />
             <span className="text-xs w-8 text-center">{viewSettings.zoomLevel}%</span>
             <ZoomIn className="h-4 w-4 text-gray-500 cursor-pointer hover:text-blue-600" onClick={() => handleZoom(10)} />
          </div>
        ...
        <div className="flex items-center gap-2">
           <div className="flex items-center gap-2 mr-2">
             ...
             />
             <Label htmlFor="show-weekend" className="text-xs">周末</Label>
           </div>
           ...
           <div className="flex items-center gap-2 mr-4">
             ...
             />
             <Label htmlFor="show-today" className="text-xs">今日</Label>
           </div>

           <Button variant="outline" size="sm" className="h-8 gap-1" onClick={() => setConfigOpen(true)}>
             <Settings className="h-3.5 w-3.5" />
             配置
           </Button>
           <Button size="sm" className="h-8 gap-1 bg-blue-600 hover:bg-blue-700" onClick={handleAddTask}>
      ...
      <div className="flex flex-1 overflow-hidden">
        {/* 左侧列表 */}
        <div className="border-r bg-white z-10 flex flex-col shadow-[4px_0_24px_rgba(0,0,0,0.02)]" style={{ width: SIDEBAR_WIDTH }}>
              ...
              {renderData.rows.map((row) => {
                if (row.type === 'group') {
                  return (
                    ...
                    >
                      {row.expanded ? <ChevronDown className="h-3 w-3 mr-1" /> : <ChevronRight className="h-3 w-3 mr-1" />}
                      {row.name} 
                      <Badge variant="secondary" className="ml-2 text-[10px] h-4 px-1">{row.count}</Badge>
                    </div>
                  )
                } else {
                  // 任务行
                    ...
                    >
                      <div className="flex-1 min-w-0">
                        <div className="text-sm text-gray-900 truncate font-medium flex items-center justify-between">
                          <span className="truncate">{formatDisplayValue(row.data[fieldMapping.title] || row.data.description || "无标题")}</span>
                          <Button 
                            variant="ghost" 
                            size="icon" 
                          ...
                          <div className="flex items-center gap-2">
                             ...
                             {row.data.status && (
                               <span className={cn(
                                 ...
                                 row.data.status === "已完成" ? "bg-green-100 text-green-700" :
                                 row.data.status === "进行中" ? "bg-blue-100 text-blue-700" : "bg-gray-100 text-gray-600"
                               )}>
                                 {row.data.status}
                               </span>
                             )}
                            ...
                            if (progress !== null) {
                              return (
                                <div className="flex items-center gap-2 w-full">
                                  ...
                                  <span className="text-[10px] text-gray-400 min-w-[24px] text-right">{progress}%</span>
                                </div>
                              )
                            }
                            return null
        ...
        <div className="flex-1 flex flex-col overflow-hidden relative">
          ...
          <ScrollArea className="h-full w-full" orientation="both" viewportRef={scrollContainerRef} onScroll={handleScroll}>
            <div className="relative" style={{ width: totalWidth, height: Math.max(renderData.totalHeight, 500) }}>
                  ...
                  return (
                    <div 
                      ...
                      className={cn(
                        ...
                        viewSettings.showWeekend && isWeekend && viewSettings.timeScale === 'day' && "bg-slate-50/60",
                        viewSettings.showToday && isToday && viewSettings.timeScale === 'day' && "bg-blue-50/30"
                      )}
              ...
              {/* 时间轴头 - 双层结构 */}
              <div className="sticky top-0 z-30 flex flex-col bg-white border-b shadow-sm" style={{ height: HEADER_HEIGHT }}>
                {/* 第一层：年份/月份/季度 */}
                <div className="flex border-b border-gray-100 h-8 items-center bg-gray-50/30 text-xs text-gray-500 font-medium">
                  {(() => {
                    ...
                    periods.forEach((period) => {
                      let label = ""
                      if (viewSettings.timeScale === 'day') label = format(period, "yyyy年 M月", { locale: zhCN })
                      else if (viewSettings.timeScale === 'week') label = format(period, "yyyy年 M月", { locale: zhCN })
                      else if (viewSettings.timeScale === 'month') label = format(period, "yyyy年", { locale: zhCN })
                      else if (viewSettings.timeScale === 'quarter') label = format(period, "yyyy年", { locale: zhCN })
                      else label = "" // 年视图不需要顶层
                    ...
                    return (
                      ...
                      >
                        <span className="font-medium scale-90">
                          {viewSettings.timeScale === 'year' ? format(period, "yyyy") :
                           viewSettings.timeScale === 'month' ? format(period, "MMM") :
                           format(period, "d")}
                        </span>
                        <span className="text-[10px] text-gray-400 scale-75 transform origin-center">
                          {viewSettings.timeScale === 'day' ? format(period, "EEE", { locale: zhCN }) :
                           viewSettings.timeScale === 'week' ? `W${format(period, "w")}` : 
                           viewSettings.timeScale === 'year' ? '' : format(period, "yyyy")}
                        </span>
                      </div>
                    )
              ...
              {/* 依赖线层 */}
              <svg className="absolute top-0 left-0 w-full h-full pointer-events-none z-10" style={{ top: HEADER_HEIGHT }}>
                <defs>
                   ...
                   return (
                     <div
                       ...
                       className={cn(
                         ...
                         // 颜色逻辑
                         row.data.priority === "重要紧急" ? "bg-red-100 border-red-200 text-red-800" :
                         row.data.priority === "紧急不重要" ? "bg-orange-100 border-orange-200 text-orange-800" :
                         "bg-blue-100 border-blue-200 text-blue-800",
                         // 选中高亮
                         highlightedTask === row.data.id && "ring-2 ring-gray-400/50 ring-offset-1 z-30 shadow-md"
                       )}
                       style={{
                         left: row.left,
                         ...
                         >
                           ...
                           <div className="relative space-y-2">
                             ...
                             <div className="font-semibold text-sm text-gray-900 break-words">
                               {formatDisplayValue(row.data[fieldMapping.title] || row.data.description || "无标题")}
                             </div>
                             ...
                             <div className="flex items-center gap-2 text-xs text-gray-500 font-medium">
                               <span>
                                 {(() => {
                                   const start = row.data[fieldMapping.startDate] || row.data.fields?.[fieldMapping.startDate]
                                   const end = row.data[fieldMapping.endDate] || row.data.fields?.[fieldMapping.endDate]
                                   if (!start && !end) return "无日期"
                                   const s = start ? format(new Date(start.replace(/\//g, "-")), "yyyy/MM/dd") : "?"
                                   const e = end ? format(new Date(end.replace(/\//g, "-")), "yyyy/MM/dd") : "?"
                                   return `${s} - ${e}`
                                 })()}
                               </span>
                               {(() => {
                             ...
                             <div className="flex flex-wrap gap-2 pt-1">
                               {(() => {
                                 ...
                                 if (fieldMapping.group && fieldMapping.group !== 'none') {
                                   const val = formatDisplayValue(row.data[fieldMapping.group] || row.data.fields?.[fieldMapping.group])
                                   if (val) fields.push({ label: "分组", value: val, color: "bg-purple-50 text-purple-700 border-purple-100" })
                                 }
                                 // 颜色字段
                                 if (fieldMapping.color && fieldMapping.color !== 'none' && fieldMapping.color !== fieldMapping.group) {
                                   const val = formatDisplayValue(row.data[fieldMapping.color] || row.data.fields?.[fieldMapping.color])
                                   if (val) fields.push({ label: "状态", value: val, color: "bg-orange-50 text-orange-700 border-orange-100" })
                                 }
                                 // 依赖关系
                                 if (fieldMapping.dependencies && fieldMapping.dependencies !== 'none') {
                                   const deps = row.data[fieldMapping.dependencies] || row.data.fields?.[fieldMapping.dependencies]
                                   if (deps && Array.isArray(deps) && deps.length > 0) {
                                     fields.push({ label: "依赖", value: `${deps.length}个前置任务`, color: "bg-gray-100 text-gray-700 border-gray-200" })
                                   }
                                 }
                                 
                                 return fields.map((f, i) => (
                                   <div key={i} className={cn("px-2 py-0.5 rounded text-[10px] border font-medium", f.color)}>
                                     {f.value}
                                   </div>
                                 ))
                               })()}
              ...
              {viewSettings.showToday && (
                   ...
                   style={{ 
                     left: (() => {
                       ...
                       const periodIndex = periods.findIndex((p, i) => {
                         ...
                         let limit
                         switch (viewSettings.timeScale) {
                            case 'year': limit = addYears(p, 1); break;
                            case 'quarter': limit = addQuarters(p, 1); break;
                            case 'month': limit = addMonths(p, 1); break;
                            case 'week': limit = addWeeks(p, 1); break;
                            default: limit = addDays(p, 1);
                         }
                ...
                >
                  <div className="bg-red-500 text-white text-[10px] px-1 py-0.5 rounded absolute -top-6 -left-4">Today</div>
                </div>
              )}
      ...
      <Dialog open={!!selectedTask} onOpenChange={(open) => !open && setSelectedTask(null)}>
        <DialogContent className="max-w-4xl max-h-[90vh] overflow-y-auto">
          ...
          {selectedTask && (() => {
            const task = data.priorityGroups.flatMap(g => g.tasks).find(t => t.id === selectedTask)
            if (!task) return null
