...
export function AssignmentBoard({ data, onTaskClick, onAddUser }: AssignmentBoardProps) {
  ...
  const { getSortedUsers, deleteUser, reorderUsers, userOrder, setUserOrder, addTask } =
    useTaskStore();
  ...
  const getTasksByUser = useCallback(() => {
    return users.reduce<Record<string, Task[]>>((acc, user) => {
      acc[user.id] = data.priorityGroups.flatMap((group) =>
        group.tasks.filter((task) => task.assignee?.id === user.id),
      );
      return acc;
    }, {});
            ...
            <div className="grid grid-cols-4 items-center gap-4">
              ...
              <Select
                ...
                onValueChange={(value) =>
                  setNewTask({
                    ...newTask,
                    status: value as '待开始' | '进行中' | '已完成' | '已停滞',
                  })
                }
              >
            ...
            <div className="grid grid-cols-4 items-center gap-4">
              ...
              <Select
                ...
                onValueChange={(value) =>
                  setNewTask({
                    ...newTask,
                    priority: value as '重要紧急' | '紧急不重要' | '重要不紧急',
                  })
                }
              >
                <SelectTrigger className="col-span-3">
...
function SortableUserColumn({
  ...
  return (
    <div
      ref={setNodeRef}
      style={style}
      className="w-80 flex-shrink-0 border-r border-gray-200 flex flex-col h-full"
    >
      <div className="p-3 border-b border-gray-200 flex items-center justify-between flex-shrink-0">
        <div className="flex items-center">
          ...
          <Avatar className="h-8 w-8 mr-2">
            <AvatarFallback className={`${getUserBgColor(user)} text-white`}>
              {user.name.slice(0, 2)}
            </AvatarFallback>
          </Avatar>
          <div>
    ...
    >
      ...
      <div className="space-y-2 mb-2">
        ...
        <div className="flex items-center text-xs">
          ...
          <div className="flex items-center">
            <CheckCircle
              className={`h-3.5 w-3.5 mr-1 ${task.completed ? 'text-green-500' : 'text-yellow-500'}`}
            />
