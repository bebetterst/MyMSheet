                ...
                (editMode ? (
                  <div onClick={(e) => e.stopPropagation()}>
                    <Select
                      value={task.status}
                      onValueChange={(value) =>
                        handleTaskUpdate(task.id, { status: value as Task['status'] })
                      }
                    >
                      <SelectTrigger className="h-8">
                        <SelectValue />
                ...
                (editMode ? (
                  <div onClick={(e) => e.stopPropagation()}>
                    <Select
                      value={task.priority}
                      onValueChange={(value) =>
                        handleTaskUpdate(task.id, { priority: value as Task['priority'] })
                      }
                    >
                      <SelectTrigger className="h-8">
                        <SelectValue />
          ...
          <div className="ml-20 grid grid-cols-2 gap-4">
            <div className="space-y-2">
              ...
              <div className="text-sm whitespace-pre-line max-h-40 overflow-y-auto">
                <EditableCell
                  value={task.summary}
                  onSave={(value) => handleTaskUpdate(task.id, { summary: value })}
                />
              </div>
            </div>
            <div className="space-y-2">
...
export default function TaskManagementTable({
  ...
  const handleDragEnd = (event: DragEndEvent) => {
    ...
    if (sourceGroupId && activeIndex !== -1 && overGroupId && overIndex !== -1) {
      ...
      // 提示用户
      const message =
        sourceGroupId === overGroupId ? '任务顺序已更新' : `任务已移动至"${overGroupId}"优先级组`;
  ...
  return (
    <div className="flex flex-col h-screen max-h-screen">
          ...
          <div className="bg-gray-50 border-b border-gray-200 p-2 flex items-center justify-between flex-shrink-0">
            ...
            <div className="flex items-center space-x-2">
              ...
              <Button
                variant="outline"
                size="sm"
                onClick={() => setIsFieldManagementDialogOpen(true)}
              >
                                  ...
                                  {headerOrder.map((fieldId) => {
                                    ...
                                    return (
                                      <SortableHeaderCell
                                        ...
                                        field={field}
                                        onResize={(width) =>
                                          handleColumnResizeCallback(field.id, width)
                                        }
                                      />
                                    );
                                  })}
                                ...
                                .map((field) => (
                                  ...
                                  >
                                    ...
                                    <div
                                      className="absolute right-0 top-0 h-full w-4 cursor-col-resize flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity duration-150"
                                      onMouseDown={(e) =>
                                        handleColumnResizeStart(e, field, (width) =>
                                          handleColumnResizeCallback(field.id, width),
                                        )
                                      }
                                    >
      ...
      <FieldManagementDialog
        open={isFieldManagementDialogOpen}
        onOpenChange={setIsFieldManagementDialogOpen}
      />
      <AddTaskDialog
