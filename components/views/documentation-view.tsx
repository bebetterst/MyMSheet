...
export function DocumentationView() {
  ...
  const { applyTemplateDefaults, views, applySavedView, renameView, deleteView, defaultViewId, setDefaultView, groupBy, sortConfig, filterConfig, viewConfig, headerOrder, visibleFields } = useTaskStore()
          ...
          <div className="space-y-6">
            ...
            <div>
              ...
              <div className="flex items-center space-x-2 mb-3">
                ...
                <Button size="sm" variant="outline" onClick={() => selectedViewId && applySavedView(selectedViewId)}>应用</Button>
                <Button size="sm" variant="outline" onClick={() => selectedViewId && setDefaultView(selectedViewId)}>设为默认</Button>
              </div>
              ...
              <div className="space-y-2">
                {views.map((v) => (
                  <div key={v.id} className="flex items-center justify-between border rounded-md p-2">
                    ...
                    <div className="flex items-center space-x-2">
                      <Input className="h-8 w-44" placeholder="重命名" value={renameId === v.id ? renameText : ""} onChange={(e) => { setRenameId(v.id); setRenameText(e.target.value) }} />
                      <Button size="sm" variant="outline" onClick={() => { if (renameId === v.id && renameText.trim()) { renameView(v.id, renameText.trim()); setRenameId(""); setRenameText("") } }}>重命名</Button>
                      <Button size="sm" variant="outline" onClick={() => deleteView(v.id)}>删除</Button>
                      <Button size="sm" variant="outline" onClick={() => applySavedView(v.id)}>应用</Button>
                    </div>
                  </div>
                ))}
              </div>
