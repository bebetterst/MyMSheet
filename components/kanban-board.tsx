...
export function KanbanBoard({ data, onTaskClick, onTaskUpdate, onAddTask }: KanbanBoardProps) {
  ...
  const [tasks, setTasks] = useState(() => {
    // 将任务按状态分组
    const tasksByStatus = {
      已停滞: data.priorityGroups.flatMap((group) =>
        group.tasks.filter((task) => task.status === '已停滞'),
      ),
      待开始: data.priorityGroups.flatMap((group) =>
        group.tasks.filter((task) => task.status === '待开始'),
      ),
      进行中: data.priorityGroups.flatMap((group) =>
        group.tasks.filter((task) => task.status === '进行中'),
      ),
      已完成: data.priorityGroups.flatMap((group) =>
        group.tasks.filter((task) => task.status === '已完成'),
      ),
    };
  ...
  const findContainer = (id: string) => {
    ...
    return Object.keys(tasks).find((key) =>
      tasks[key as keyof typeof tasks].some((task) => task.id === id),
    );
  };
  ...
  const handleDragEnd = (event: DragEndEvent) => {
    ...
    // 如果是在同一列内拖拽（上下排序）
    if (activeContainer === overContainer) {
      const activeIndex = tasks[activeContainer as keyof typeof tasks].findIndex(
        (item) => item.id === active.id,
      );
      const overIndex = tasks[overContainer as keyof typeof tasks].findIndex(
  ...
  return (
    ...
    >
      ...
      <Dialog
        ...
        onOpenChange={(open) => !open && setIsAddTaskDialogOpen(null)}
      >
        <DialogContent className="sm:max-w-[500px]">
          <DialogHeader>
            <DialogTitle>添加新任务到 {isAddTaskDialogOpen}</DialogTitle>
            <DialogDescription className="sr-only">在该状态列下创建一个新任务。</DialogDescription>
          </DialogHeader>
          <div className="grid gap-4 py-4">
            ...
            <div className="grid grid-cols-4 items-center gap-4">
              ...
              <Input
                ...
                onChange={(e) =>
                  setNewTask({ ...newTask, expectedEndDate: e.target.value.replace(/-/g, '/') })
                }
                className="col-span-3"
              />
            </div>
  ...
  return (
    <div className="w-80 flex-shrink-0 border-r border-gray-200 flex flex-col h-full">
      <div className="p-3 border-b border-gray-200 flex items-center justify-between flex-shrink-0">
        <div className="flex items-center">
          <Badge className={`${colorClass} border-0 text-gray-800 mr-2 badge-hover-effect`}>
            {title}
          </Badge>
